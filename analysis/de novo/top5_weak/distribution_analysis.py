#!/usr/bin/env python3
"""
De novo vs GT 分布对比：LLPS 关键驱动特征

对比三组：
1. De novo ITO (509 条 top-1)
2. GT Top 5% (509 条原始序列)
3. Missing_0 全量 (10,165 条)
...
def kl_divergence_continuous(p_vals, q_vals, n_bins=50):
    """Compute KL(P||Q) for continuous distributions via histogram binning."""
...
def compute_aa_composition(seqs):
    """Compute average AA frequency across sequences."""
...
def plot_feature_comparison(feat_dict, feature_names, output_path, kl_dict=None):
    """Violin plots: ITO / Gen / Random (left) + GT (right) per feature."""
    ...
    labels = list(feat_dict.keys())  # ITO, Gen, Random, GT
    ...
    plt.suptitle('LLPS Key Feature Distribution: De Novo vs GT', fontsize=14, y=1.01)
    plt.tight_layout()
...
def plot_aa_composition(comp_dict, aa_list, output_path):
    """Bar chart comparing AA composition (ITO / Gen / Random / GT)."""
    ...
    ax.set_title('Amino Acid Composition: De Novo vs GT', fontsize=13)
...
def main():
    OUT_DIR = '/data/yanjie_huang/LLPS/predictor/PhaseFlow/analysis/de novo/top5_weak/distribution_analysis'
    os.makedirs(OUT_DIR, exist_ok=True)

    # Load data
    ito = pd.read_csv('/data/yanjie_huang/LLPS/predictor/PhaseFlow/analysis/de novo/top5_weak/de_novo_ito_results/ito_top1.csv')
    gen = pd.read_csv('/data/yanjie_huang/LLPS/predictor/PhaseFlow/analysis/de novo/top5_weak/de_novo_results/candidates.csv')
    gt = pd.read_csv('/data/yanjie_huang/LLPS/predictor/PhaseFlow/analysis/de novo/top5_weak/ground_truth.csv')

    denovo_ito_seqs = ito['sequence'].tolist()
    gt_seqs = gt['gt_sequence'].tolist()

    # Sample 1 per phase from Gen (fair comparison with ITO 509)
    ...
    gt_label = 'GT Top5%'
    print('\n' + '=' * 90)
    print(f'{"Feature":<22} {"De Novo ITO":>14} {"De Novo Gen":>14} {"GT Top5%":>14} {"KL(ITO||GT)":>12} {"KL(Gen||GT)":>12}')
    print('=' * 90)
