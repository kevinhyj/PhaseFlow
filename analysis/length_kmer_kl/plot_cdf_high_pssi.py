#!/usr/bin/env python3
"""
绘制序列长度的累积分布函数（CDF）- 仅 mean PSSI > 0.3 的序列
随机序列采样相同条数
"""
...
high_mask = gt_df['mean_pssi'] > 0.3
n_high = high_mask.sum()
print(f"筛选 mean PSSI > 0.3: {n_high} 条")

gt_high = gt_df[high_mask]
gen_high = gen1x_df[high_mask.values]

np.random.seed(42)
rand_high = random_df.sample(n=n_high, random_state=42)

gt_lengths = gt_high['AminoAcidSequence'].str.len().values
gen_lengths = gen_high['generated_sequence'].str.len().values
rand_lengths = rand_high['AminoAcidSequence'].str.len().values
...
output_path = 'outputs/cdf_length_comparison_high_pssi.png'
