...
DEFAULT_DATA_PATH="${PHASEFLOW_DATA_PATH:-${PROJECT_DIR}/data/phase_diagram_original_scale.csv}"
...
show_help() {
    ...
    echo "  -c, --config PATH         Config path or name under configs/ (default: ${DEFAULT_CONFIG})"
    echo "  -d, --data PATH           Training CSV (default: PHASEFLOW_DATA_PATH or data/phase_diagram_original_scale.csv)"
    ...
    find "${PROJECT_DIR}/configs" -maxdepth 2 -name "*.yaml" -printf "  %P\n" 2>/dev/null | sort
}
...
cd "$PROJECT_DIR"
if "$FOREGROUND"; then
    python -u experiments/train.py "${TRAIN_ARGS[@]}" 2>&1 | tee "$LOG_FILE"
else
    nohup python -u experiments/train.py "${TRAIN_ARGS[@]}" >> "$LOG_FILE" 2>&1 &
    PID=$!
