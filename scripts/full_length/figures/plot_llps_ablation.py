#!/usr/bin/env python3
"""Plot LLPS ablation metrics from a tabular evaluation export."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Plot LLPS ablation metrics from a CSV file.")
    parser.add_argument("--input", type=Path, required=True)
    parser.add_argument("--output-dir", type=Path, required=True)
    parser.add_argument("--label-column")
    parser.add_argument("--metric", action="append", default=["auprc", "auroc", "mcc_at_0.5"])
...
def main() -> int:
    ...
    for axis, column in zip(axes, metrics):
        ...
        axis.scatter(rows[column], rows[label].astype(str), color="#f585bf", s=48, zorder=2)
