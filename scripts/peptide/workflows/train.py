...
from phaseflow.peptide.data import create_dataloader, collate_fn
from phaseflow.peptide.utils import (
    set_seed,
    load_config,
    save_config,
    get_logger,
...
def parse_args():
    ...
    parser.add_argument(
        "--config",
        type=str,
        default="configs/peptide/peptide.yaml",
        help="Path to config file"
    )
    parser.add_argument(
        "--data_path",
        type=str,
        default="artifacts/data/peptide/phase_diagram_original_scale.csv",
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Optional validation CSV. If omitted, a validation split is made from --data_path."
    )
    parser.add_argument(
        "--test_path",
        type=str,
        default=None,
        help="Optional test CSV. If omitted, a test split is made from --data_path."
    )
    parser.add_argument(
        "--output_dir",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Optional directory for training-curve figures. Defaults to <run_output>/figures."
    )
    parser.add_argument(
        "--resume",
        type=str,
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Optional override for data.missing_threshold (-1 to disable). "
             "When omitted, the value in the YAML config is used (or -1 if absent)."
    )
...
def main():
    ...
    if args.missing_threshold is None:
        args.missing_threshold = int(data_config.get('missing_threshold', -1))
    val_path = args.val_path or data_config.get('val_path')
    test_path = args.test_path or data_config.get('test_path')
    train_split = 'all' if (val_path and test_path) else 'train'
    ...
    logger.info(f"Validation data: {val_path or args.data_path}")
    logger.info(f"Test data: {test_path or args.data_path}")

    val_loader = create_dataloader(
        val_path or args.data_path,
        batch_size=config['training']['batch_size'],
        split='all' if val_path else 'val',
        num_workers=config['data']['num_workers'],
        tokenizer=tokenizer,
        max_seq_len=config['model']['max_seq_len'],
        use_npz=False,
        normalize_phase=False,
        missing_threshold=-1 if val_path else args.missing_threshold,
    )

    test_loader = create_dataloader(
        test_path or args.data_path,
        batch_size=config['training']['batch_size'],
        split='all' if test_path else 'test',
        num_workers=config['data']['num_workers'],
        tokenizer=tokenizer,
        max_seq_len=config['model']['max_seq_len'],
        use_npz=False,
        normalize_phase=False,
        missing_threshold=-1 if test_path else args.missing_threshold,
    )
    ...
    visual_dir = Path(args.visual_dir) if args.visual_dir else output_dir / "figures"
