...
OUT_DIR = ROOT / 'artifacts/results/peptide/analysis/flow_vs_ddpm_ito/benchmark_results'
...
def main():
    ...
    parser.add_argument('--fm_checkpoint',   type=str,
                        default=str(ROOT / 'outputs_set' / 'output_set_flow32_missing15' / 'best_model.pt'))
    ...
    parser.add_argument('--input_csv',  type=str,
                        default=str(ROOT / 'artifacts' / 'data' / 'peptide' / 'test_set.csv'))
    ...
    ddpm_checkpoint = Path(args.ddpm_checkpoint)
    input_csv = Path(args.input_csv)
    output_dir = Path(args.output_dir)
    if not fm_checkpoint.is_absolute():
        fm_checkpoint = ROOT / fm_checkpoint
    if not ddpm_checkpoint.is_absolute():
        ddpm_checkpoint = ROOT / ddpm_checkpoint
    ...
    # Load models
    print('\n[Flow Matching]')
    fm_model, fm_type = load_model(str(fm_checkpoint), device)
    print('\n[DDPM]')
    ddpm_model, ddpm_type = load_model(str(ddpm_checkpoint), device)
    ...
    csv_path = output_dir / 'benchmark_optimization_results.csv'
    df_out.to_csv(csv_path, index=False)
    print(f'\nResults saved to {csv_path}')

    # Plot
    plot_time_curve(fm_times, ddpm_times, output_dir / 'benchmark_time_curve.png')
    ...
    plot_spearman_bar(fm_spearman, fm_std, ddpm_spearman, ddpm_std,
                      checkpoint_rounds, output_dir / 'benchmark_spearman_bar.png')
