"""
PhaseFlow Inference-Time Optimization — Bottom 5% 强相分离 (Strong LLPS)

对 Bottom 5% 强相分离相图 (PSSI最负=相分离最强)，每个生成 200 个候选序列，
用 Flow 反向预测相图 + Spearman 打分，每个相图取 Top-1。

Usage:
    cd 
    python "scripts/peptide/analysis/de_novo/bottom5_strong/de_novo_ito.py" --gpu 0
    python "scripts/peptide/analysis/de_novo/bottom5_strong/de_novo_ito.py" --gpu 0 --n_candidates 50 --num_samples 10  # 快速测试
"""
...
def main():
    parser = argparse.ArgumentParser(description='De Novo ITO for Bottom 5%')
    parser.add_argument('--checkpoint', type=str,
                        default='outputs_set/output_set_flow32_missing15/best_model.pt')
    parser.add_argument('--input_csv', type=str,
                        default='artifacts/data/peptide/by_missing/missing_0.csv')
    parser.add_argument('--output_dir', type=str,
                        default='artifacts/results/peptide/analysis/de_novo/bottom5_strong/de_novo_ito_results')
    ...
    if args.num_samples is not None:
        weak_idx = weak_idx[:args.num_samples]
    ...
    for i in tqdm(range(len(target_phases)), desc="ITO"):
        ...
        row = {
            ...
            'source_data_idx': int(weak_idx[i]),
    ...
    # Summary
    print("\n" + "=" * 60)
    print(f"ITO Summary (Bottom {args.bottom_percent}%, {args.n_candidates} candidates/phase)")
    print("=" * 60)
