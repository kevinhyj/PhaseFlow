"""
序列多样性分析：Top 5% 弱相分离相图 (Weak LLPS, PSSI最正=相分离最弱)
...
Usage:
    cd 
    python scripts/peptide/analysis/de_novo/top5_weak/sequence_diversity/diversity_analysis.py --gpu 5 --n_samples 100
"""
...
CHECKPOINT = "outputs_set/output_set_flow32_missing15/best_model.pt"
MISSING0_CSV = "artifacts/data/peptide/by_missing/missing_0.csv"
GROUP_COLS = [f'group_{i}{j}' for i in range(1, 5) for j in range(1, 5)]
OUTPUT_DIR = "artifacts/results/peptide/analysis/de_novo/top5_weak/sequence_diversity"
...
def analyze_one_phase(model, tokenizer, phase_tensor, n_samples, gen_batch, device):
    ...
    aa_var = compositions.var(axis=0).mean()  # 20 维方差的均值
    ...
    return {
        ...
        'pred_phase_var': pred_phases.var(axis=0).mean(),  # 生成序列预测相图的方差
    }
...
def main():
    ...
    parser.add_argument('--n_phases', type=int, default=None, help='分析多少个相图（None=全部509）')
    ...
    q95 = np.percentile(mean_pssi, 95)
    strong_idx = np.where(mean_pssi >= q95)[0]

    if args.n_phases is not None:
        strong_idx = strong_idx[:args.n_phases]

    print(f"Top 5% 弱相分离 (Weak LLPS): {len(strong_idx)} 条, mean_pssi >= {q95:.4f}")
    print(f"每个相图生成 {args.n_samples} 条序列")

    # 逐个相图分析
    results = []
    for idx in tqdm(strong_idx, desc="Analyzing diversity"):
    ...
    # ================================================================
    print("\n" + "=" * 60)
    print(f"序列多样性分析 (Top 5%, n_samples={args.n_samples})")
    print("=" * 60)
