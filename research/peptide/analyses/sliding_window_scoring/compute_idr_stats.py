...
WINDOW_SIZE = 10
DEFAULT_OUTPUT_DIR = ROOT / 'artifacts' / 'results' / 'peptide' / 'idr_analysis'
DEFAULT_WINDOW_DIR = ROOT / 'artifacts' / 'results' / 'peptide' / 'sliding_window_scoring'
DEFAULT_IDR_CSV = ROOT / 'artifacts' / 'data' / 'full_length' / 'human_idr.csv'
...
def main():
    parser = argparse.ArgumentParser(description='Compare IDR and non-IDR window score distributions.')
    parser.add_argument('--nn_jsonl', type=str, default=str(DEFAULT_WINDOW_DIR / 'nn_windows.jsonl'))
    parser.add_argument('--xgb_jsonl', type=str, default=str(DEFAULT_WINDOW_DIR / 'xgb_windows.jsonl'))
    parser.add_argument('--phaseflow_jsonl', type=str, default=str(DEFAULT_WINDOW_DIR / 'protein_window_scores.jsonl'))
    parser.add_argument('--idr_csv', type=str, default=str(DEFAULT_IDR_CSV))
    ...
    # 加载数据
    print("\n1. 加载数据...")
    nn_results = load_results(args.nn_jsonl)
    xgb_results = load_results(args.xgb_jsonl)
    pf_results = load_results(args.phaseflow_jsonl)
    idr_dict = load_human_idr(args.idr_csv)
    ...
    plt.tight_layout()
    fig1.savefig(output_dir / 'fig1_nn_distribution.png', dpi=150, bbox_inches='tight')
    ...
    plt.tight_layout()
    fig2.savefig(output_dir / 'fig2_xgboost_distribution.png', dpi=150, bbox_inches='tight')
    ...
    plt.tight_layout()
    fig3.savefig(output_dir / 'fig3_phaseflow_distribution.png', dpi=150, bbox_inches='tight')
