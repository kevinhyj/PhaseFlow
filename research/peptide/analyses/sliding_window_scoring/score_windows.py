...
DEFAULT_PROTEINS_FILE = ROOT / 'artifacts' / 'data' / 'full_length' / 'unique_proteins.csv'
DEFAULT_MODEL_PATH = ROOT / 'outputs' / 'output_missing0_flow32' / 'best_model.pt'
DEFAULT_OUTPUT_DIR = ROOT / 'artifacts' / 'results' / 'peptide' / 'sliding_window_scoring'
...
class SlidingWindowScorer:
    ...
    def __init__(self, model_path: Path, window_size: int = 10, device='cuda:0', window_batch_size=64):
    ...
    def score_protein(self, sequence):
        ...
        n_windows = max(0, seq_len - self.window_size + 1)

        if n_windows <= 0:
            return []

        # 收集所有窗口序列
        window_seqs = [sequence[i:i+self.window_size] for i in range(n_windows)]
...
def main():
    ...
    parser.add_argument('--input_csv', type=str, default=str(DEFAULT_PROTEINS_FILE),
                        help='CSV with Entry and Sequence columns')
    parser.add_argument('--checkpoint', type=str, default=str(DEFAULT_MODEL_PATH),
                        help='PhaseFlow checkpoint path')
    parser.add_argument('--output_dir', type=str, default=str(DEFAULT_OUTPUT_DIR),
                        help='Output directory')
    parser.add_argument('--window-size', type=int, default=10,
                        help='Sliding window size in amino acids')
    ...
    logger.info(f"Window size: {args.window_size} aa")
    logger.info(f"Device: {device}")
    logger.info(f"Window batch size: {args.window_batch}")
    logger.info("=" * 60)

    # 初始化打分器
    scorer = SlidingWindowScorer(
        model_path=checkpoint,
        window_size=args.window_size,
        device=device,
        window_batch_size=args.window_batch,
    )
    scorer.load_models()
    ...
    logger.info(f"Proteins >= {args.window_size} aa: {len(seq_data)}")
    logger.info(f"Skipped (non-standard AA): {skipped_count}")

    # 估计总窗口数
    est_windows = sum(len(s['sequence']) - args.window_size + 1 for s in seq_data)
