#!/usr/bin/env python3
"""
PhaseFlow (DDPM Version) Inference-Time Optimization

对每个输入相图生成 N 个候选序列，用 seq→phase 预测相图并计算相关性，
选择相关性最高的序列作为最终输出。

使用 DDPM (cosine beta schedule, DDIM 50-step) 模型: fw=32, lm=1

Usage:
    # Test a single target phase diagram
    python research/peptide/experiments/generate_with_optimization_ddpm.py --n_candidates 10 --num_samples 1 --gpu 0

    # 处理前500个相图
    python generate_with_optimization_ddpm.py --n_candidates 100 --num_samples 500 --gpu 0

    # 处理全部相图
    python generate_with_optimization_ddpm.py --n_candidates 100 --gpu 0
"""
...
def load_model(checkpoint_path: str, device: torch.device):
    ...
    print(f"  dim={config['model']['dim']}, depth={config['model']['depth']}, "
          f"diffusion_type={config.get('training', {}).get('diffusion_type', 'ddpm')}, "
...
def generate_and_score_candidates(
    ...
    temperature: float, max_len: int, ddim_steps: int, device: torch.device
):
    """
    ...
    Args:
        ...
        ddim_steps: DDIM 采样步数（DDPM 推理使用）
    ...
    # === Step 3: 批量预测相图 (DDPM: euler method) ===
    all_pred_phases = []
...
def main():
    parser = argparse.ArgumentParser(description='PhaseFlow DDPM Inference-Time Optimization')
    parser.add_argument('--checkpoint', type=str,
                        default=str(ROOT / 'outputs_ddpm' / 'output_set_ddpm_flow32_missing15' / 'best_model.pt'))
    parser.add_argument('--input_csv', type=str,
                        default=str(ROOT / 'artifacts' / 'data' / 'peptide' / 'by_missing' / 'missing_0.csv'))
    parser.add_argument('--output_dir', type=str,
                        default=str(ROOT / 'artifacts' / 'results' / 'peptide' / 'generated' / 'missing0_optimized_ddpm'))
    ...
    parser.add_argument('--ddim_steps', type=int, default=50,
                        help='DDIM sampling steps for DDPM generation')
    ...
    # Print summary stats
    print("\n" + "=" * 60)
    print("Optimization Summary (DDPM fw=32, lm=1)")
    print("=" * 60)
