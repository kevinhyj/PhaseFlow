...
```yaml
# configs/set_ddpm_cosine_missing15.yaml
model:
  dim: 256
  depth: 6
  heads: 8
...
```bash
# 标准训练
bash scripts/train.sh -c configs/set_flow32_missing15.yaml -g 0

# 从断点恢复
bash scripts/train.sh -c configs/set_flow32_missing15.yaml -g 0 --resume /path/to/checkpoint.pt
```
