...
If `--val` and `--test` are omitted, `experiments/train.py` creates train/val/test splits from
`--data`.
...
The underlying Python entrypoint is `experiments/predict_seq2phase.py`.
...
- `scripts/kill_training.sh`: interactively terminate training processes.
