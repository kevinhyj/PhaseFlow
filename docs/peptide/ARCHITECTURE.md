...
├── analyses/sliding_window_scoring/
...
```bash
# 标准训练
bash scripts/peptide/train.sh -g 0 -c bs2048_lr0.0008_flow32_20260123.yaml

# 层次化训练（只用 missing≤11 的数据）
bash scripts/peptide/train.sh -g 0 -c flow32_missing11_20260310.yaml -t 11

# 参数覆盖
bash scripts/peptide/train.sh -g 0 -c config.yaml -b 1024 -l 0.0004 -e 100
```
