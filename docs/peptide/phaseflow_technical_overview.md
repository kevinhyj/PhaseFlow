...
```bash
cd .
bash scripts/peptide/train.sh -c <config_file> -g <gpu_id>
```
...
```bash
# Flow Matching (默认)
bash scripts/peptide/train.sh -c set_flow32_missing15.yaml -g 0

# DDPM
bash scripts/peptide/train.sh -c set_ddpm_cosine_missing15.yaml -g 0

# 纯语言模型 (flow_weight=0)
bash scripts/peptide/train.sh -c set_lm32_missing15.yaml -g 0
```
...
```yaml
# configs/peptide/set_ddpm_cosine_missing15.yaml
model:
  dim: 256
  depth: 6
  heads: 8
...
```bash
# 标准训练
bash scripts/peptide/train.sh -c configs/peptide/set_flow32_missing15.yaml -g 0

# 从断点恢复
bash scripts/peptide/train.sh -c configs/peptide/set_flow32_missing15.yaml -g 0 --resume /path/to/checkpoint.pt
```
